...
9. `read_until` loop safety relies on a non-obvious invariant
  - **Problem because:** `ensure_initialized` is called once before the loop, not per iteration. The safety of `read_once` (never reading into an empty slice) depends on the limit check exiting the loop before `end` can reach `buf.len()`. This invariant — that the single upfront initialization provides enough headroom for the entire loop — is correct but non-obvious and has no comment.
  - **Not a problem because:** If you see this as a problem, you can just read the code and see what it does to realize that there is no problem. The name `ensure_initialized` is a hint of the expected semantic.
