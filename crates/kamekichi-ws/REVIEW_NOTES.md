...
1. `saturating_add` overflow masking
...
1. `consume` panic/clamp semantics
...
1. `slice()` can index into initialized-but-never-written memory
...
1. `read_until` limit counts pending bytes, not total bytes read
...
1. `ensure_initialized` allocates more than the requested `target`
...
1. Test coverage appears to lack dedicated unit tests for specific paths
...
1. `clear` vs `compact`-when-all-consumed semantic overlap
...
1. No `#[must_use]` on `maybe_compact`
...
1. Error types lack `Display` / `std::error::Error` impls
...
1. `read_until` gives `f` one free call on pre-existing data before the limit check
...
1. `fill_from` with `need = 0` is a silent no-op
...
1. `MIN_READ_SLICE` / `MIN_READ_HEADROOM` naming
...
1. `maybe_compact` doc wording said "exceeds both"
...
1. `read_until` doc doesn't hint at how the callback manages cursor state
